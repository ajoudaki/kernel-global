...
% The proposition shows that the kernel sequence $\rho_\ell$ converges to a deterministic sequence. We can relax the conditions on the weights to allow any distribution with zero mean and variance $1/d,$ such as uniform distribution. 
...
%  \thomas{I would suggest to merge the two propositions. Perhaps one can even simply re-state known results, e.g.~that feedforward NNs are Gaussian processes in the infinite width limit, which covers the two data point case as a special one.}
% \end{proof}
...
% \end{align}
% where $\kappa$ is defined in Definition~\ref{def:kernel_map}. \thomas{Maybe we can compactify by first defining the map $\kappa$ and then using it in the propositions. It feels a bit redundant right now.} 
...
\begin{align*}
    % \frac{dx}{x(1-x)} = -(1-c_1^2) dt \\
    % \implies \int \frac{dx}{x(1-x)} = \int -(1-c_1^2) dt \\
    \ln \frac{|x(t)|}{|1-x(t)|} = -t (1-c_1^2) + C.
    % \implies \frac{x}{1-x} = C' \exp(-t(1-c_1^2))\\
   % \implies \frac{x(t)}{1-x(t)} = \frac{x(0)}{1-x(0)} \exp(-t(1-c_1^2)).
\end{align*}
...
% This solution explains the main reason that $\rho/(1-\rho)$ formula that appears on both sides of the theorem statement. 
