...
\begin{align*}
\E\,\he_k(X)\he_l(Y) = \rho^n \delta_{kl}, && X, Y\sim \mathcal N\left(0, \begin{pmatrix} 1 & \rho \\ \rho & 1 \end{pmatrix}
 \right).
\end{align*}
\end{lemma}
...
% \subsubsection{Theorem 1: Residual Connections}

\begin{theorem}
\label{thm:residual_kernel_map}
Let us assume the activation function $\phi$ satisfies $\E \phi(X)^2 = 1,\, X\sim N(0,1),$ and let $\kappa_\phi$ denote its kernel map. Consider the MLP with residual strength $t$ given by
\begin{align*}
h^\ell = \sqrt{1 - r^2} \, \phi\left( \frac{1}{\sqrt{d}}W^\ell  h^{\ell-1} \right) + r \, \frac{1}{\sqrt{d}}P^\ell  h^{\ell-1},
\end{align*}
where $W^\ell$ and $P^\ell$ are independent weight matrices of dimension $d \times d$ with entries drawn i.i.d.~from a zero-mean, unit-variance distribution, and $r \in [0, 1)$ modulates the strength of the residual connection. Then the residual kernel map is given by
\begin{equation}
\kappa_\res(\rho) = (1 - r^2) \kappa_\phi(\rho) + r^2 \rho.
\end{equation}

Furthermore, the fixed points of $\kappa_\res(\rho)$ are the same as those of $\kappa(\rho)$. For $r \in [0, 1)$, the convergence behavior remains in the same case as for $\kappa(\rho)$, but the convergence rate is affected by $r$.
\end{theorem}

\paragraph{Proof idea.} First observation is that due to independence of $W^\ell$ and $P^\ell,$ the cross terms in the joint kernel disappear. Thus, the residual connection acts as a linear combination of the transformed input and the input itself, leading to the modified kernel map $\kappa_\res(\rho)$. We can easily verify that any fixed point $\rho^*$ of $\kappa_\phi,$ is also a fixed point of $\kappa_\res,$ and because the identity map is neither attracting nor repulsive, the attracting qualities of $\rho^*$ will transfer from $\kappa_\phi$ to $\kappa_\res.$ Finally, 

\paragraph{Implications.} Theorem~\ref{thm:residual_kernel_map} reveals that residual connections modify the kernel map by blending the original kernel map with the identity function, weighted by the residual strength $r$. This adjustment affects the convergence rate to the fixed point but not the fixed point itself. Specifically, stronger residual connections (larger $r$) slow down the convergence, indicating a slower rate that the network ``forgets'' the similarity between inputs. 
...
The following theorem characterizes the joint activation and normalization kernel, depending on the order between two and type of normalization layer used. 

\begin{theorem}
\label{thm:normalization_kernel_map}
...
% Then, for each of the cases that $\psi \in \{RN\circ \phi, \phi\circ LN, \phi\circ RN\}$ the joint kernel map is given by $\kappa(\rho),$ and in the case that $\psi = LN\circ \phi,$ the joint kernel map is given by $(\kappa(\rho)-\kappa(0))/(1-\kappa(0)).$
\begin{align*}
    \kappa_\psi = \begin{cases}
         \frac{\kappa_\phi-\kappa_\phi(0)}{1-\kappa_\phi(0)} & \psi=LN\circ \phi\\
        \kappa_\phi & \text{otherwise},\\
    \end{cases}
\end{align*}
...
% - \textbf{Pre-Activation Normalization and Post-Activation RMS}: The kernel map is rescaled, which may accelerate or decelerate convergence depending on the shape of $\kappa(\rho)$.
% - \textbf{Post-Activation LN}: The kernel map is both shifted and scaled, potentially leading to different fixed points or convergence behaviors.

% Understanding these effects is crucial for designing neural network architectures that maintain desirable representational properties across layers.
...
\begin{proof}[Proof of Theorem~\ref{thm:normalization_kernel_map}]
